...
import { X, RefreshCcw, LogOut, ChevronRight, Menu } from 'lucide-react';
...
export default function LeadsDashboard() {
    ...
    <>
      <div className="p-6 border-b border-[#1C1C2E]">
        <div className="flex items-center justify-between gap-3 mb-6">
          ...
          <button onClick={closeSidebar} className="md:hidden p-1 text-[#8B8BAD] hover:text-white rounded-lg hover:bg-[#13131F]">
            <X className="w-5 h-5" />
          </button>
        </div>
  ...
  return (
    <>
      {/* Mobile header bar */}
      <div className="md:hidden fixed top-0 left-0 right-0 z-50 h-14 bg-[#0B0B12] border-b border-[#1C1C2E] flex items-center justify-between px-4">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 bg-gradient-to-br from-amber-500 to-orange-700 rounded-lg flex items-center justify-center font-display font-bold text-white text-sm">ND</div>
          <span className="font-display font-bold text-white text-lg">Lead Dashboard</span>
        </div>
        <button onClick={() => setIsSidebarOpen(true)} className="p-2 text-[#8B8BAD] hover:text-white rounded-lg hover:bg-[#13131F]">
      ...
      <AnimatePresence>
        {isSidebarOpen && (
          <motion.div className="fixed inset-0 z-60 flex md:hidden" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            {/* Backdrop */}
            ...
            <motion.aside 
              ...
              className="relative z-10 w-72 bg-[#0D0D16] border-r border-[#1C1C2E] flex flex-col h-full overflow-y-auto"
            >
              <SidebarContent />
            </motion.aside>
          </motion.div>
      ...
      <div className="flex h-screen overflow-hidden bg-[#0B0B12] text-[#F1F1F8]">
        ...
        <aside className="hidden md:flex w-60 bg-[#0D0D16] border-r border-[#1C1C2E] flex-col shrink-0">
          <SidebarContent />
        </aside>

        {/* Main Content */}
        <main className="flex-1 flex flex-col min-w-0 pt-14 md:pt-0">
          <header className="hidden md:flex h-16 border-b border-[#1C1C2E] items-center justify-between px-6 bg-[#0B0B12] z-10 shrink-0">
          ...
          <div className="flex-1 overflow-auto p-4 md:p-6">
            <div className="md:hidden mb-4 flex justify-end">
               <button onClick={fetchLeads} className="flex items-center gap-2 px-3 py-1.5 text-sm bg-[#13131F] border border-[#1C1C2E] text-white rounded-lg">
                 <RefreshCcw className={'w-4 h-4' + (loading ? ' animate-spin' : '')} /> Refresh
               </button>
            </div>
            
            <div className="bg-[#13131F] border border-[#1C1C2E] rounded-xl overflow-hidden flex flex-col min-w-0">
              <div className="overflow-x-auto w-full">
                <table className="w-full text-left text-xs md:text-sm whitespace-nowrap min-w-[800px]">
                  <thead className="bg-[#0D0D16] border-b border-[#1C1C2E] text-[#8B8BAD] uppercase tracking-wider text-[10px] md:text-[11px] font-bold">
                    <tr>
                      <th className="px-4 md:px-6 py-4">Company</th>
                      <th className="px-4 md:px-6 py-4">Person & Role</th>
                      <th className="px-4 md:px-6 py-4">Workflow Area</th>
                    ...
                    {leads.map(lead => (
                      <tr key={lead.id} className="hover:bg-[#17172A] transition-colors cursor-pointer" onClick={() => setSelectedLead(lead)}>
                        <td className="px-4 md:px-6 py-4">
                          ...
                          <div className="text-[#8B8BAD] text-[10px] md:text-xs">{lead.industry}</div>
                        </td>
                        <td className="px-4 md:px-6 py-4">
                          <div className="text-white">{lead.name}</div>
                          <div className="text-[#8B8BAD] text-[10px] md:text-xs">{lead.jobTitle}</div>
                        </td>
                        <td className="px-4 md:px-6 py-4 text-[#F1F1F8]">{lead.workflowArea}</td>
                        <td className="px-4 md:px-6 py-4">
                           <span className={'inline-flex items-center px-2 md:px-2.5 py-1 rounded-full text-[10px] md:text-xs font-semibold border ' + (statusColors[lead.status] || 'bg-gray-800 text-gray-300 border-gray-700')}>
                            {lead.status}
                          </span>
                        </td>
                        <td className="px-4 md:px-6 py-4">
                          <div className="flex items-center gap-2">
                            <div className="w-12 md:w-16 h-1.5 bg-[#0B0B12] rounded-full overflow-hidden">
                              <div className="h-full bg-amber-500 rounded-full" style={{ width: ((calcTotalScore(lead.opportunityScores) / 60) * 100) + '%' }} />
                            </div>
                            <span className="text-[10px] md:text-xs font-medium text-[#8B8BAD]">{calcTotalScore(lead.opportunityScores)}/60</span>
                          </div>
                        </td>
                        <td className="px-4 md:px-6 py-4 text-[#8B8BAD] text-[10px] md:text-xs">
                          {new Date(lead.submittedAt).toLocaleDateString()}
                        </td>
                        <td className="px-4 md:px-6 py-4 text-right">
                          <ChevronRight className="w-4 h-4 md:w-5 md:h-5 text-[#52526E] inline-block" />
                        </td>
                      </tr>
                    ))}
                    {leads.length === 0 && !loading && (
                      <tr>
                        <td colSpan={7} className="px-4 md:px-6 py-12 text-center text-[#8B8BAD]">No leads found.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
          ...
          {selectedLead && (
            <>
              ...
              <motion.div
                initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }}
                transition={{ type: 'spring', damping: 25, stiffness: 200 }}
                className="fixed right-0 top-0 bottom-0 w-full md:max-w-2xl bg-[#0D0D16] border-l border-[#1C1C2E] z-[80] flex flex-col shadow-2xl"
              >
                <div className="h-14 md:h-16 border-b border-[#1C1C2E] flex items-center justify-between px-4 md:px-6 bg-[#0B0B12] shrink-0">
                ...
                <div className="flex-1 overflow-auto p-4 md:p-6 space-y-6 md:space-y-8">
                  {/* Header Info */}
                  <div className="bg-[#13131F] border border-[#1C1C2E] rounded-xl p-4 md:p-6">
                    <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4 mb-6">
                      <div>
                        <h3 className="text-xl md:text-2xl font-bold text-white">{selectedLead.company}</h3>
                        <p className="text-[#8B8BAD] text-sm md:text-base">{selectedLead.industry} • {selectedLead.companySize} employees</p>
                      </div>
                      <select
                        value={selectedLead.status}
                        onChange={(e) => setSelectedLead({ ...selectedLead, status: e.target.value as any })}
                        className="bg-[#0B0B12] border border-[#1C1C2E] text-white text-sm rounded-lg px-3 py-2 outline-none focus:border-amber-500 self-start"
                      >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                      ...
                      <div><span className="text-[#52526E] block mb-1">Email</span><span className="text-white break-all">{selectedLead.email}</span></div>
                  ...
                  <div className="bg-[#13131F] border border-[#1C1C2E] rounded-xl p-4 md:p-6">
                    <div className="flex items-end justify-between mb-6">
                      ...
                      <div className="text-2xl md:text-3xl font-display font-bold text-white">{calcTotalScore(selectedLead.opportunityScores)}<span className="text-[#52526E] text-base md:text-lg">/60</span></div>
                    </div>
                        ...
                        return (
                          <div key={factor}>
                            ...
                            <input
                              ...
                              className="w-full accent-amber-500 h-1.5 bg-[#0B0B12] rounded-lg appearance-none cursor-pointer"
                            />
                          </div>
                        )
                      })}
                ...
                <div className="p-4 md:p-6 border-t border-[#1C1C2E] bg-[#0B0B12] shrink-0">
                  <button
